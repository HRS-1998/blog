...
[vue 单元测试官方文档](https://vue-test-utils.vuejs.org/zh/)
...
module.exports = {
  ...
  //transformIgnorePatterns: ['node_modules/(?!ct-dart)']
  moduleFileExtensions: ['js', 'jsx', 'json', 'vue', 'ts', 'tsx'],
  moduleNameMapper: {
    '^@/(.*?.?(js|vue)?|)$': '<rootDir>/src/$1', // @路径转换，例如：@/components/Main.vue -> rootDir/src/components/Main.vue
    '\\.(jpg|jpeg|png|gif|eot|otf|webp|svg|ttf|woff|woff2|mp4|webm|wav|mp3|m4a|aac|oga)$':
      '<rootDir>/tests/unit/__mock__/fileMock.js', // 模拟加载静态文件
    '\\.(css|less|scss|sass)$': '<rootDir>/tests/unit/__mock__/styleMock.js' // 模拟加载样式文件
  },
...
使用localVue注入公共组件 注入数据 使用 nextTick进行异步

```
import { mount, createLocalVue } from '@vue/test-utils';
import Vue from 'vue';
import ElementUI from 'element-ui';
import CtDart from 'ct-dart';
...
describe('Breadcrumb', () => {
  it('Breadcrumb', () => {
    const wrapper = mount(Breadcrumb, { localVue });
...
describe('table', () => {
  it('table', () => {
    const wrapper = mount(Table, {
      propsData: {
        data: [
          { date: '11', name: 'xiao', address: 'sdf' },
          { date: '112', name: 'xiao1', address: 'sdf2' }
        ]
      },
      localVue
    });
    return Vue.nextTick().then(function() {
      expect(wrapper.html()).toMatch('xiao1');
    });
  });
});
```
